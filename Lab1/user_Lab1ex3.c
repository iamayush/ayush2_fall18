...
#define MAX_TIME 800 // slowest speed
#define MIN_TIME 25 // fastest speed
#define TIME_STEP 25 // incr/decrement in time

char newprint = 0;
unsigned int timecnt = 0;
unsigned int fastcnt = 0;
unsigned int pttrnTime = 0;

int s1 = 0, s2 = 0, s3 = 0, s4 = 0; // switch trigger counters
int LEDtime = 400; // LED stays ON for these many ms
...
void main(void) {
    ...
    P2IES |= 0x30;  // port 2 pins 4,5 are pulled up
    P2IES &= ~0xC0; // port 2 pins 6,7 are pulled down
    P2IE  |= 0xF0;  // Enable port 2 pins 4,5,6,7 interrupts
    ...
    while(1) {
        ...
        if (newprint)  {
            // UART_printf("s1 %d s2 %d s3 %d s4 %d \n\r",s1,s2,s3,s4);
            UART_printf("LEDtime %d\n\r",LEDtime);
...
__interrupt void Timer_A (void)
{
    ...
    pttrnTime++; // timer for LED pattern

    if (pttrnTime == LEDtime){
        switch (P1OUT){
        case 0x01:
            P1OUT = 0x02;
            break;
